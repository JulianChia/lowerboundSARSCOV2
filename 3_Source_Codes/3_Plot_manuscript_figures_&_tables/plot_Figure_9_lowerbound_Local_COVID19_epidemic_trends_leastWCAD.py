...
def plot_COVID19_curves(CADS):
    ...
    def add_zoom_region(ax, xdates, ye, yp):
        ...
        axins.bar(xdates, ye, color='C0', alpha=0.5)
        axins.bar(xdates, yp, color='C1', alpha=0.5)
    ...
    for n, (seed, v) in enumerate(CADS.items()):
        ...
        y_covid19 = v['covid_days_ncases']
        score = round(float(v['score'][1]), 2)
        # Plot data
        axes[n].bar(x_dates, y_emp_covid19, label='Empirical', color=colors[0],
                    alpha=0.5)
        axes[n].bar(x_dates, y_covid19, label=f'{seed}, WCAD={score}',
                    color=colors[1], alpha=0.5)
        # Configure Y-axis
        axes[n].get_yaxis().set_label_text(
            label='Number of Daily Local COVID-19 Cases',
        ...
        # Add Zoom Insert
        add_zoom_region(ax[n], x_dates, y_emp_covid19, y_covid19)
...
if __name__ == "__main__":
    ...
    source1 = results_data / 's1' / 's1_stp2_wcad_wcad.csv'
    source2 = results_data / 's2' / 's2_stp2_wcad_wcad.csv'
    source3 = results_data / 's3' / 's3_stp2_wcad_wcad.csv'
    wcadfiles = (source1, source2, source3)
    seeds = ('Seed1', 'Seed2', 'Seed3')
    WCADS = {seeds[n]: read_CAD_WCAD(source=i) for n, i in enumerate(wcadfiles)}
    plot_COVID19_curves(WCADS)
    # seed = 91023483923
    # seed = 66576402094
    # seed = 343090589475
